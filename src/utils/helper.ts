...
public static calculatedAveragePrice = (max: number, min: number, current: number): [boolean, number, number] => {
    const mediumPrice = (min + max) / 2;
    const differenzaPercentuale: number = ((mediumPrice - current) / mediumPrice) * 100;
